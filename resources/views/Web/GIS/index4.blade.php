    ...
    <div id="sidebar" class="sidebar collapsed">
        ...
        <div class="sidebar-content">
            ...
            <div class="sidebar-pane" id="profile">
                ...
                <div class="all-desa mt-3">
                    <!-- Checkbox untuk menampilkan semua layer desa -->
                    <input type="checkbox" id="show-all-desa-checkbox" data-group="desa">
                    <label for="show-all-desa-checkbox">Tampilkan Semua Desa</label>
                    <hr class="">
                </div>

                <div class="desa mt-3" id="checkbox-desa">
...
<script>
    ...
    function filterEvents() {
        ...
        var allEventCheckboxes = document.querySelectorAll('input[data-group="event"]');

        allEventCheckboxes.forEach(function(checkbox) {
        ...
        if (selectedYear === 'all') {
            ...
            var selectedEventCheckboxes = document.querySelectorAll('#event-checkboxes-' + selectedYear +
                ' input[data-group="event"]');
            selectedEventCheckboxes.forEach(function(checkbox) {
                checkbox.parentNode.style.display = 'block';
            });
        }
    ...
    Event listener untuk checkbox event
    var eventCheckboxes = document.querySelectorAll('input[data-group="event"]');
    ...
    function initMap() {
        ...
        function addGeoJSONLayer(url) {
            $.getJSON(url, function(json) {
                ...
                // Iterasi melalui setiap fitur GeoJSON
                json.features.forEach(function(feature) {
                    if (feature.properties && feature.properties.NAMOBJ) {
                        var NAMOBJ = feature.properties.NAMOBJ;
                        // Tambahkan NAMOBJ ke dalam daftar jika belum ada
                        if (!uniqueNAMOBJs.includes(NAMOBJ)) {
                            uniqueNAMOBJs.push(NAMOBJ);
                        }
                    }
                });

                // Buat checkbox untuk setiap NAMOBJ
                uniqueNAMOBJs.forEach(function(NAMOBJ) {
                    var checkbox = document.createElement('input');
                    checkbox.type = 'checkbox';
                    checkbox.value = NAMOBJ;
                    checkbox.id = 'checkbox-' + NAMOBJ; // ID unik untuk setiap checkbox
                    checkbox.setAttribute('data-group', 'desa');
                    checkbox.onclick = function() {
                        pilihDesa(this, NAMOBJ);
                    };

                    var label = document.createElement('label');
                    label.htmlFor = 'checkbox-' + NAMOBJ;
                    label.appendChild(document.createTextNode(NAMOBJ));
                    label.classList.add('ml-1');

                    var container = document.getElementById('checkbox-desa');
                    container.appendChild(checkbox);
                    container.appendChild(label);
                    container.appendChild(document.createElement('br'));

                    // Muat dan simpan layer tanpa menambahkannya ke peta
                    $.getJSON('/assets-sig/assets/shape/Desa/Geo/' + NAMOBJ + '.geojson', function(
                        json) {
        ...
        $.getJSON('/geojson-files', function(files) {
            ...
            // Tambahkan layer kab_ketapang setelah semua layer desa dimuat
        ...
        function pilihDesa(checkbox, NAMOBJ) {
            if (checkbox.checked) {
                // Tambahkan layer GeoJSON dari NAMOBJ yang dipilih ke dalam peta
                if (allLayers[NAMOBJ]) {
                    allLayers[NAMOBJ].addTo(map);
                    markerClusters[NAMOBJ] = allLayers[NAMOBJ];
                    allLayers[NAMOBJ].bringToBack(); // Pastikan layer desa berada di belakang
                }
            } else {
                // Hapus layer GeoJSON dari NAMOBJ yang dipilih dari peta
                if (markerClusters[NAMOBJ]) {
                    map.removeLayer(markerClusters[NAMOBJ]);
                    delete markerClusters[NAMOBJ];
                }
            }

        }

        document.getElementById('show-all-desa-checkbox').addEventListener('change', function() {
            var allCheckboxes = document.querySelectorAll('input[data-group="desa"]');
            var isChecked = this.checked;

            allCheckboxes.forEach(function(checkbox) {
                checkbox.checked = isChecked;
                pilihDesa(checkbox, checkbox.value);
            });
        });


        // Event listener untuk checkbox desa individual
        var desaCheckboxes = document.querySelectorAll('input[data-group="desa"]');
        desaCheckboxes.forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                // Dapatkan nilai desa yang dipilih
                var desa = this.value;
                // Lakukan sesuatu sesuai dengan nilai desa yang dipilih
                console.log("Desa yang dipilih:", desa);
            });
        });
        ...
        L.control.Legend({
            ...
            legends: [{
                ...
                },
                {
                    label: " : Batas Desa",
                    type: "image",
                    url: "{{ url('/') }}/assets-web2/assets/images/icon/dashed-line.png",
                },
                {
                    label: ": Nama Desa",
                    type: "image",
                    url: "{{ url('/') }}/assets-web2/assets/images/icon/shape.png",
                }
    ...
    function resetMap() {
        ...
        map.setView([-1.790597, 110.410990], 10, {
            animate: true,
            duration: 2,
        });

        // Reset checked status for checkboxes
        // Reset checked status for checkboxes with data-group="event"
        var checkboxes = document.querySelectorAll('input[data-group="event"]');
    ...
    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener("change", function() {
            if (checkbox.checked) {
                var selectedEventId = checkbox.value;
                eventMarkers.forEach(function(eventMarker) {
                    if (eventMarker.event_id !== selectedEventId) {
                        eventMarker.setOpacity(0);
                    }
                });
            } else {
    ...
    function filterMarkers() {
        // Ambil semua checkbox event yang dicek
        var checkedEventCheckboxes = document.querySelectorAll('input[data-group="event"]:checked');

        // Kosongkan semua marker tanaman
        clearTanamanMarkers();

        // Set opasitas semua marker event kembali ke 1
        eventMarkers.forEach(function(eventMarker) {
            eventMarker.setOpacity(1);
        });

        // Periksa setiap checkbox event yang dicek
        checkedEventCheckboxes.forEach(function(checkbox) {
            var eventId = checkbox.value;

            // Temukan marker event yang sesuai dengan event id
            var eventMarker = eventMarkers.find(function(marker) {
                return marker.event_id == eventId;
            });

            // Jika ditemukan marker event, arahkan peta ke marker event tersebut
            if (eventMarker) {
                map.flyTo(eventMarker.getLatLng(), 10, {
                    duration: 2, // Durasi animasi dalam detik
                    easeLinearity: 0.5
                });
            }
